...
sv = Service('_feedback_', manage_priv=priv.SUPERUSER, help_='[来碗布丁] 后接反馈内容 联系维护组')

_max = 3
lmt = DailyNumberLimiter(_max)
EXCEED_NOTICE = f'没有布丁了，请明早5点后再来！'
...
async def feedback(bot, ev: CQEvent):
    ...
    if not text:
        await bot.send(ev, "请发送来碗布丁+您要反馈的内容~", at_sender=True)
    else:
